...
func TestCopyStruct(t *testing.T) {
	...
	if err := copier.Copy(&user, employee); err == nil {
...
func TestCopyFromStructToSlice(t *testing.T) {
	...
	if err := copier.Copy(&user, employees); err != nil && len(employees) != 0 {
		t.Errorf("Copy to unaddressable value should get error")
	}

	if copier.Copy(&user, &employees); len(employees) != 1 {
	...
	employees2 := &[]Employee{}
	if copier.Copy(user, &employees2); len(*employees2) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees3 := []*Employee{}
	if copier.Copy(user, &employees3); len(employees3) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if copier.Copy(user, &employees4); len(*employees4) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
...
func TestCopyFromSliceToSlice(t *testing.T) {
	...
	if copier.Copy(users, &employees); len(employees) != 2 {
	...
	employees2 := &[]Employee{}
	if copier.Copy(&users, &employees2); len(*employees2) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees3 := []*Employee{}
	if copier.Copy(users, &employees3); len(employees3) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if copier.Copy(users, &employees4); len(*employees4) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
